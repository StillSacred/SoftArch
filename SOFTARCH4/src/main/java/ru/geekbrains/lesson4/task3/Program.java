...
public class Program {
    ...
    public static void main(String[] args) {
        ...
        if (mobileApp.buyTicket("1452367896541230")) {
...
class TicketProvider {
    ...
    private void validateClientId(int clientId) {
        if (clientId <= 0) throw new RuntimeException("Некорректный ID пользователя.");
    }
    // Проверка номера карты
    private void validateCardNo(String cardNo) {
        if (cardNo.length() != 16) throw new RuntimeException("Некорректный номер карты.");
        if (cardNo.contains(".")) throw new RuntimeException("Некорректный номер карты.");
        try {
    ...
    private void validateQrcode(String qrcode) {
        if (qrcode == null) throw new RuntimeException("Некорректный QR-код.");
    }
}
